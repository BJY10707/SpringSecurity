...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChainConfigure(HttpSecurity http) throws Exception {
        ...
        // #2. 로그인관리: <form> 태그를 사용한 로그인(form-login) 관련 설정
        }).formLogin(login -> {
            // 로그인 페이지 경로 설정(로그인 페이지에 해당되는 핸들러 매핑이 존재해야함)
            ...
            // 로그인 실패 시, 해당 예외를 처리할 핸들러 지정(직접 제작한 핸들러사용)
            login.failureHandler(authFailHandler);
        // #3. 로그아웃 관리: 로그아웃 요청시 관련 처리설정
        }).logout(logout -> {
            // 로그아웃 경로 설정
        ...
        // #4. 세션관리 : 애플리케이션 내 세션 관리 설정
        }).sessionManagement(session -> {
            // 동시 세션 수(허용개수)를 1개로 제한
            session.maximumSessions(1);
            // 세션 만료 시 이동할 페이지의 URL경로
